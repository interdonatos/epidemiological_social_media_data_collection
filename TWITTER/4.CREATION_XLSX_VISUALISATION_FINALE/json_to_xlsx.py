...
with open("output2.json") as infile:
    jstring = infile.read()
data = json.loads(re.sub('$',']}', jstring))

with open('output2.json','w') as mon_fichier:
    json.dump(data, mon_fichier)
"""
with open("output2.json", encoding="utf-8") as data_file:
...
class FileManagement:
    def __init__(self):
        self.json_file_name="output2.json"
        self.excel_file_name="output2.xlsx"
        self.array_id_=[]
        self.array_date=[]
        self.array_tweet=[]
    ...
    def read_text_file(self):
        try:
            with open(self.json_file_name, encoding="utf-8") as data_file:
                ...
                data=json.load(data_file)
                #print(data)
                for each_axis in data['Tweets']:
                    id_=str(each_axis["id"])
                    date=str(each_axis["date"])
                    tweet=str(each_axis["tweet"])
                    ...
                    self.array_sum.append(id_+date+tweet+spatial_entities+thematic_entities)
    ...
    def save_to_xlsx(self):
        ...
        for index, value in enumerate(self.array_id_):
            ...
            worksheet.write(index,2,self.array_tweet[index])
